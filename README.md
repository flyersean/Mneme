...
Mneme is a proxy that sits between an AI agent and its model backend, archives every conversation into searchable memory, and injects relevant past context on each turn. It grades its own epistemic honesty through provenance, not answer-correctness, and evolves strategies through a self-improving loop.

> **This branch (`unified_mneme`) is the full build.** The strategy / self-improving layer (strategy learning, capability-edge → overcome, novel-procedure save, belief evolution, learning + thinking modes) is **enabled by default** (`MNEME_MEMORY_ONLY=0`). The `main` branch runs the memory-only build with that layer off.
...
The model tags its sources (`[source: X]` / `[guess]`) and is graded on *honesty*, not answer-correctness:

- Provenance grading — "I don't know" beats fabrication.
...
Strategies are retrieved by their **source chunk** — the chunk that produced them — not by a hand-maintained taxonomy, so a strategy goes wherever its source chunk is relevant.
...
- `unified_mneme` — **current development branch** (this README). The full build: strategy/self-improving layer enabled by default (`MNEME_MEMORY_ONLY=0`).
- `main` — **memory-only build**: the same latest code with the strategy/self-improving layer disabled by default (`MNEME_MEMORY_ONLY=1`). Memory retrieval, provenance grading, and the full toolset stay on. Start here if you only want memory. Install it with `MNEME_BRANCH=main` (see install step).
